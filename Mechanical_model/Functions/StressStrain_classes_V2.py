...
class StrainVolume:
    ...
    def _BoundaryPerturbation(self,BoundaryTolerance,tol=1e-3):
        ...
        for jj in tqdm(range(self.vertices.shape[0]), desc='Observation Points Boundary Check'):
            ...
            #--> sélection des points de la face xmin du bloc
    ...
    def LoadBlocks(self,BLOCKS,PoissonRatio=0.25,BiotCoef=1.0,ShearModulus=6e9):
        ...
        #pour chaque bloc, self.vertices contient les 3 coordonnées de chacun des 8 sommets
    ...
    def PointValues(self,device='cpu',GreenFunction=False):
        ...
        for ii in tqdm(range(len(self.vertices)),desc='Computing Disp and σ'):
            ...
            for jj, vertx in enumerate(block):
                ...
                # -- Stresses --
                # σXX
                #print(x[...,0].device, x[...,1].device, Sσ.device)
                #print (self.ν)
                #print (rm.device)
                #print (ζm.device)
                #print (rp.device)
                #print (ζp.device)
                #print (type(self.obspoints_inBlock.int()))
                #print (self.obspoints_inBlock.int().device)
                self.σ[ii,:,0] += Sσ[jj]*(
...
class Groningen:
    def __init__(self,ReservoirFolder):
        ...
        self.BLOCKS['dP/Strain']  = np.zeros(self.BLOCKS['Z'].shape)#(self.RES['PRESSURE']['TimeEvolution'][:,:,-1].flatten())
    ...
    def SingleBlock(self,PATH,index=0,sep=5,extr=100):
        ...
        POINTS_np[:,1] = BLOCK['Y']+BLOCK['wY']/2 #Y est fixe, avec sa valeur au mileu du bloc : coupe trasversale selon Y + wY/2
    ...
    def DepthSlice(self,PATH,depth=0.0,mu=0.3,Strike=270,Dip=85,smoothing=6.0,GreenFunction=False,verbrose=True):
        ...
        if verbrose:
          ...
          #Contraintes normales, de cisaillement et de Coulomb
          #Contrainte normale : n.σ.n
          Stress_faultnormal  = np.dot(np.dot(Stress,Fault_normal),Fault_normal)
          #Composante tangentielle : σ.n - (n.σ.n)n (contraintes totales sur la face de normale n moins la partie normale)
          Stress_faultshear   = np.sqrt(np.sum(np.dot(Stress,Fault_normal)**2,axis=-1) - Stress_faultnormal**2)
          #obspoints_inBlock_value contient les valeurs de pression dans le réservoir
          #Coulomb stress : égal à la valeur de cisaillement - un coefficient fois (la conrainte normale - la pression de pore)
          Stress_faultcoulomb = (Stress_faultshear - mu*(Stress_faultnormal-self.StrainVolume.obspoints_inBlock_value.numpy()))/1e6
          ...
          #Max shear : différence entre la valeur propre max et la valeur propre min divisée par 2
          Stress_maxshear  = 0.5*(np.max(np.linalg.eigvals(np.nan_to_num(Stress)),axis=-1) - np.min(np.linalg.eigvals(np.nan_to_num(Stress)),axis=-1))
          #Max normal : contrainte normale au niveau de (σmax + σmin)/2 , sert à calculer le coulomb stress au point de maximum shear stress
          Stress_maxnormal = 0.5*(np.max(np.linalg.eigvals(np.nan_to_num(Stress)),axis=-1) + np.min(np.linalg.eigvals(np.nan_to_num(Stress)),axis=-1))
          #Calcul du coulom stress au point de maximum shear stress (qui semble ne pas correspondre exactement ave le point de max coulomb stress)
          Stress_maxcoulomb = (Stress_maxshear - mu*(Stress_maxnormal-self.StrainVolume.obspoints_inBlock_value.numpy()))/1e6
          Stress_maxcoulomb[Stress_maxcoulomb==np.inf]=0
          ...
          #PLOT DU MAXIMUM COULOMB STRESS
          ...
          #IDEM MAIS EN LISSANT AVEC UN KERNAL GAUSSIEN
          kernel = Gaussian2DKernel(smoothing)
